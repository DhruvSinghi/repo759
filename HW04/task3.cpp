...
void getAcc(const double pos[][3], const double mass[], double acc[][3], int N) {
    ...
    // TODO:
    #pragma omp single
    memset(acc,0.0,sizeof(double) * N * 3);
...
int main(int argc, char *argv[]) {
    ...
    {
    double velCM_temp[3] = {0.0, 0.0, 0.0}; // Thread-local storage for each thread
    double totalMass_temp = 0.0;

    #pragma omp for
    for (int i = 0; i < N; i++) {
        double mass_i = mass[i];
        velCM_temp[0] += vel[i][0] * mass_i;
        velCM_temp[1] += vel[i][1] * mass_i;
        velCM_temp[2] += vel[i][2] * mass_i;
        totalMass_temp += mass_i;
    }

    // Use a critical section to accumulate results into shared velCM
    #pragma omp critical
    {
        velCM[0] += velCM_temp[0];
        velCM[1] += velCM_temp[1];
        velCM[2] += velCM_temp[2];
        totalMass += totalMass_temp;
    }
    }
